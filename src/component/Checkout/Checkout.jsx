...
function Checkout(props) {
    ...
    const handleCheckout = async () => {
        ...
        if (!isValid) return
        postData('6086709cdc52ab1ae999e882')
        set_redirect(true)
    }
    const postData = async (id_payment) => {
        const data_delivery = {
            fullname: fullname,
            phone: phone,
        }
        ...
        setOrderID(response_order._id)

        // Xử lý API Detail_Order
        await detailOrderAPI.post_detail_order({ cartItem: cartItem, id_order: response_order._id })
    ...
    const handleMoMo = async () => {
        const isValid = validateAll();
        if (!isValid) return
        await postData('60afcfcedc48d73138aceaf6')
        // setTimeout(() => {
        set_show_error(true)
        // }, 1500)
    }
                            ...
                            <div className="payment-accordion">
                                <div id="accordion">
                                    <div className="card">
                                        ...
                                        <div className="card-header" id="#payment-3" onClick={handleMoMo}>
                                            <h5 className="panel-title">
                                                <img src="https://developers.momo.vn/images/logo.png" width="50" />
                                                {
                                                    show_error && <MoMo orderID={orderID} />
                                                }
                                            </h5>
                                        </div>
